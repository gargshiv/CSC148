...
class Simulation:
    ...
    def run(self, start: datetime, end: datetime) -> None:
        ...
        for ride in self.all_rides:
            PriorityQueue.add(RideStartEvent(ride))
    ...
    def _update_active_rides(self, time: datetime) -> None:
        ...
        for current_ride in self.all_rides:
            ...
            if time == e and current_ride in self.active_rides:
                if ride_station_end.num_bikes < ride_station_end.capacity and ride_station_end.unocc_spots > 0:
    ...
    with open(rides_file) as file:
        for line in csv.reader(file):
            ...
            if line[1] in stations.keys() and line[3] in stations.keys():
                ...
                rides.append(
                    Ride(stations[start_station], stations[end_station],
                         (start_time,
                          end_time)))
...
class RideStartEvent(Event):
    ...
    def __init__(self, ride: Ride) -> None:
        """Initialize a new event."""
        self.ride = ride
        self.ride_time = ride.start_time
    def process(self) -> List['Event']:
        """Process this event by updating the state of the simulation.

                Return a list of new events spawned by this event.
        """
        if self.time == self.ride.start_time and self.time < self.ride.end_time:
            if self.ride.start.num_bikes > 0 and self.ride.start. \
                    unocc_spots != self.ride.start.capacity:
                self.ride.start.num_bikes -= 1
                self.ride.start.stats['starting rides'] += 1
                self.ride.start.unocc_spots += 1
                self.simulation.active_rides.append(self.ride)

                self.all_events.append(RideStartEvent)
                new_rideevent = RideEndEvent(self.ride)
...
class RideEndEvent(Event):
    ...
    def __init__(self, ride):
        self.ride = ride
        self.ending_ride_time = ride.end_time
    ...
    def process(self) -> List['Event']:
        ...
        if self.time == self.ride.end_time and self.ride in self.simulation.active_rides:
            if self.ride.end.num_bikes < self.ride.end.capacity and self.ride.end.unocc_spots > 0:
                self.ride.end.stats['ending rides'] += 1
                self.ride.end.num_bikes += 1
                self.ride.end.unocc_spots -= 1
