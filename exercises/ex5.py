...
class Tree:
    ...
    def __eq__(self, other: 'Tree') -> bool:
        ...
        if self._subtrees == [] and other._subtrees == [] and self._root == other._root:
            return True
        elif self._root == other._root:
            flag = True
            for i in range(len(self._subtrees)):
                if self._subtrees[i]._subtrees != []:
                    result = self._subtrees[i].__eq__(other._subtrees[i])
                    if result is True:
                        continue
                    else:
                        flag = False
    ...
    def to_nested_list(self) -> list:
        ...
        if self._subtrees == []:
            ...
            for i in range(len(self._subtrees)):
                if self._subtrees[i]._subtrees == []:
                    nested_list.append([self._subtrees[i]._root])
                else:
                    nested_list.append(self._subtrees[i].to_nested_list())
            return nested_list
...
class BinaryTree:
    ...
    def preorder(self) -> list:
        ...
        if self.is_empty():
            ...
            if self._left is not None:
                ans = self._left.preorder()
                lI.extend(ans)
            if self._right is not None:
                ans1 = self._right.preorder()
                lI.extend(ans1)
            return lI
    ...
    def inorder(self) -> list:
        ...
        if self.is_empty():
            ...
            li = []
            if self._left is not None:
                ans = self._left.inorder()
                li.extend(ans)
            li += [self._root]

            if self._right is not None:
                ans1 = self._right.inorder()
                li.extend(ans1)
    ...
    def postorder(self) -> list:
        ...
        if self.is_empty():
            ...
            lis = []
            if self._left is not None:
                ans = self._left.postorder()
                lis.extend(ans)
            if self._right is not None:
                ans1 = self._right.postorder()
                lis.extend(ans1)
            lis += [self._root]
            return lis
