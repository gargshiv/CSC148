...
class Game:
    ...
    def __init__(self, max_depth: int,
        ...
        Precondition:
            2 <= max_depth <= 5
        """
        number_players = random_players + len(smart_players) + num_human
        self.renderer = Renderer(number_players)
        ...
        # Creating all human players
        for player_number in range(num_human):
            random_colour = COLOUR_LIST[random.randint(0, len(COLOUR_LIST))]
            self.players.append(HumanPlayer(self.renderer, player_number, \
                                            goal(random_colour)))

        # # Creating all random players
        # for _ in range(random_players):
        #     #TODO MAKE RANDOM PLAYER INPUT CORRECT, use the player number to assign id
        #     player_id = len(self.players)
        #     self.players.append(RandomPlayer())
        #
        # # Creating all smart players
        # for player_difficulty in smart_players:
        #     #TODO make smart players using their values on the list to assign their
        #     player_id = len(self.players)
        #     self.players.append(SmartPlayer())
