...
class Block:
    ...
    def rectangles_to_draw(self) -> List[Tuple[Tuple[int, int, int],
        ...
        # TODO: WHEN INCLUDED THIS PART INSIDE THE IF, IT PASSES THE TEST, BUT MAKES THE ANIMATION AKWARD
    ...
    def swap(self, direction: int) -> None:
        ...
        if self.children is None:
            return
        if direction == 1:
            self.children[UP_RIGHT], self.children[UP_LEFT] = \
                self.children[UP_LEFT], self.children[UP_RIGHT]
            self.children[LOW_RIGHT], self.children[LOW_LEFT] = \
                self.children[LOW_LEFT], self.children[LOW_RIGHT]
        else:
            self.children[UP_RIGHT], self.children[LOW_RIGHT] = \
                self.children[LOW_RIGHT], self.children[UP_RIGHT]
            self.children[UP_LEFT], self.children[LOW_LEFT] = \
                self.children[LOW_LEFT], self.children[UP_LEFT]
    ...
    def rotate(self, direction: int) -> None:
        ...
        # Moves the list backwards or frontwards, which produces the rotation
        # effect
        self.children = [self.children[(i + index_offset) % len(self.children)]
    ...
    def update_block_locations(self, top_left: Tuple[int, int],
        ...
        if self.children is not None:
            for child_index in range(4):
                ...
                child.update_block_locations(child.position, self.size // 2)
    ...
    def flatten(self) -> List[List[Tuple[int, int, int]]]:
        ...
        if self.children is None:
            ...
            flatten_table_00 = self.children[UP_LEFT].flatten()
            flatten_table_10 = self.children[UP_RIGHT].flatten()

            flatten_table_top = []
            for line_number in range(len(flatten_table_00)):
                flatten_table_top.append(flatten_table_00[line_number] +
                                         flatten_table_10[line_number])

            # Then we assemble the bottom half part of the block
            flatten_table_01 = self.children[LOW_LEFT].flatten()
            flatten_table_11 = self.children[LOW_RIGHT].flatten()

            flatten_table_bottom = []
            for line_number in range(len(flatten_table_01)):
                flatten_table_bottom.append(flatten_table_01[line_number] +
                                            flatten_table_11[line_number])

            # Hence we assemble both halves (up and bottom)
...
def random_init(level: int, max_depth: int) -> 'Block':
    ...
    if level <= max_depth and random_number < math.exp(-0.25 * level):
        ...
        children_list = []
        for _ in range(4):
            children_list.append(random_init(level + 1, max_depth))
        random_block = Block(level, None, children_list)
    else:
        # Now we use a random number to pick up a colour
        random_numbe = random.randint(0, len(COLOUR_LIST) - 1)
        random_block = Block(level, COLOUR_LIST[random_numbe], None)
